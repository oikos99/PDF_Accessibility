...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            # ]
            #
            # final_html_path, image_report_path = process_final_html_images(
            #     html_path=final_html_path,
            ...
            # Process images in the final patron-facing HTML file.
            # This classifies images, removes high-confidence decorative images,
            # adds alt text and long descriptions where appropriate,
            # writes a QA report, and embeds the retained images as base64.
            search_roots = [
                temp_output_dir,
                os.path.dirname(final_html_path),
            ]
            ...
            # Add a deployment marker to the final HTML file.
            with open(final_html_path, "r", encoding="utf-8") as f:
                html_content = f.read()
            ...
            # Upload the final patron-facing HTML file.
            remediated_s3_key = f"remediated/{filename_base}.html"
            ...
            print(
                "[INFO] Uploaded final self-contained HTML to "
                f"s3://{bucket}/{remediated_s3_key}"
            )

            # Upload the internal QA report generated by process_final_html_images().
            # This JSON report is for staff review, not for patron delivery.
            image_report_s3_key = (
                f"remediated/{filename_base}.image-report.json"
            )
            ...
            print(
                "[INFO] Uploaded image-processing QA report to "
                f"s3://{bucket}/{image_report_s3_key}"
            )
        except Exception as e:
